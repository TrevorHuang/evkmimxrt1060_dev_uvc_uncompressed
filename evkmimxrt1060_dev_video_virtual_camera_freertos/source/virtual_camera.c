 ...
 * Variables
 ******************************************************************************/
AT_NONCACHEABLE_SECTION(uint8_t img[USB_VIDEO_VIRTUAL_CAMERA_UNCOMPRESSED_FRAME_DATA_BITS* \
...
//        }
    ...
    if (g_UsbDeviceVideoVirtualCamera.imageIndex < USB_VIDEO_VIRTUAL_CAMERA_UNCOMPRESSED_FRAME_HEIGHT *
                                                       USB_VIDEO_VIRTUAL_CAMERA_UNCOMPRESSED_FRAME_WIDTH *
                                                       USB_VIDEO_VIRTUAL_CAMERA_UNCOMPRESSED_FRAME_DATA_BITS)
    {
        ...
        memcpy((&g_UsbDeviceVideoVirtualCamera.imageBuffer[0] + sizeof(usb_device_video_mjpeg_payload_header_struct_t)),
               (uint8_t *)((uint8_t *)img + g_UsbDeviceVideoVirtualCamera.imageIndex), sendLength);
        g_UsbDeviceVideoVirtualCamera.imageBuffer = (uint8_t *)payloadHeader;
        g_UsbDeviceVideoVirtualCamera.imageIndex += sendLength;
        g_UsbDeviceVideoVirtualCamera.imageBufferLength += sendLength;

        if (g_UsbDeviceVideoVirtualCamera.imageIndex >=
...
static void USB_DeviceVideoApplicationSetDefault(void)
{
    ...
    g_UsbDeviceVideoVirtualCamera.currentMaxPacketSize = FS_STREAM_IN_PACKET_SIZE;
    ...
    memset(img, 0x88, USB_VIDEO_VIRTUAL_CAMERA_UNCOMPRESSED_FRAME_DATA_BITS* \
			USB_VIDEO_VIRTUAL_CAMERA_UNCOMPRESSED_FRAME_WIDTH*   \
			USB_VIDEO_VIRTUAL_CAMERA_UNCOMPRESSED_FRAME_HEIGHT);
}
